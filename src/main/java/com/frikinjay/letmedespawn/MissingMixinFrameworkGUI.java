...
public class MissingMixinFrameworkGUI extends GuiScreen
{
    ...
    actionPerformed(GuiButton button)
    {
        switch (button.id)
        {
            case 1:
            {
                ...
                {
                    LetMeDespawn.LOGGER.error("Couldn't open link", throwable);
                }
            } break;
            case 2:
            {
    ...
    drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        String title = new TextComponentString(LetMeDespawn.MOD_NAME + " Message").setStyle(new Style().setBold(true)).getFormattedText();
        ...
        this.fontRenderer.drawSplitString(
            new TextComponentString(
                "WARNING! " + LetMeDespawn.MOD_NAME + " has failed to load. No instance of the Mixin library detected.").getFormattedText(),
                i,
                j + 12,
                218,
                5197647);
